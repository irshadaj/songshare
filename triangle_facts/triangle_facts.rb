# Public: analyze 3 integers to determine what type of triangle they would form and some properties of each
#
# sides - array of 3 integers that describe 3 side lenghts
#
# Examples
#
#   tritri = Triangle.new(5, 5, 5)
#   tritri.recite_facts
#   #=> "This triangle is equilateral!
#        The angles of this triangle are 60,60,60"
#
# Returns evaluation of:
#    equilateral or isoceles or scalene
#    calculation of angles created by sides
#    determination if a right triangle exists
# via string output

class Triangle
  attr_accessor :side1,:side2,:side3

  def initialize(side1,side2,side3)
    @side1,@side2,@side3 = side1,side2,side3
  end

  def equilateral()
    side1 == side2 && side2 == side3
  end

  def isosceles()
    [side1,side2,side3].uniq.length == 2
  end

  def scalene()
    unless equilateral || isosceles
      true
    end
  end

  def calculate_angles(a,b,c)
    angleA = radians_to_degrees(Math.acos((b**2 + c**2 - a**2) / (2.0 * b * c)))
    angleB = radians_to_degrees(Math.acos((a**2 + c**2 - b**2) / (2.0 * a * c)))
    angleC = radians_to_degrees(Math.acos((a**2 + b**2 - c**2) / (2.0 * a * b)))

    [angleA, angleB, angleC]
  end

  def radians_to_degrees(rads)
    (rads * 180 / Math::PI).round
  end

  def recite_facts
    case
    when equilateral
      puts 'This triangle is equilateral!'
    when isosceles
      puts 'This triangle is isosceles! Also, that word is hard to type.'
    when scalene
      puts 'This triangle is scalene and mathematically boring.'
    end    
    angles = self.calculate_angles(side1,side2,side3)
    puts 'The angles of this triangle are ' + angles.join(',')

    puts 'This triangle is also a right triangle!' if angles.include? 90
    puts ''
  end


end


triangles = [
  [5, 5, 5],
  [5, 12, 13],
  [5, 4, 4]
]
triangles.each { |sides|
  tri = Triangle.new(*sides)
  tri.recite_facts
}
